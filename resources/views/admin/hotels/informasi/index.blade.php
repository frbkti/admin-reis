        ...
        <div class="card mt-3 border shadow" style="padding: 30px;">
          <div class="card-body p-4">
            <div class="d-flex justify-content-between mb-4">
              <h5 class="card-title fw-semibold">Hotel Informasi</h5>
              <!-- Button Create -->
              <a href="{{ route('admin.hotels.informasi.create') }}" class="btn btn-primary">Add Informasi</a>
            </div>
            <hr>
            <div class="table-responsive">
              <table class="table text-nowrap mb-0 align-middle">
                ...
                <tbody>
                  ...
                  <tr>
                    <td class="border-bottom-0"><h6 class="fw-semibold mb-0">{{ $hotel->id }}</h6></td>
                    <td class="border-bottom-0"><h6 class="fw-semibold mb-1">{{ $hotel->hotelName }}</h6></td>
                    <td class="border-bottom-0"><p class="mb-0 fw-normal">{{ $hotel->location }}</p></td>
                    <td class="border-bottom-0"><p class="mb-0 fw-normal">{{ $hotel->description }}</p></td>
                    <td class="border-bottom-0">
                      <img src="{{ asset($hotel->gambar) }}" alt="Gambar Hotel" class="img-fluid" style="max-width: 100px; height: auto;">
                    </td>
                    <td class="border-bottom-0"><h6 class="fw-semibold mb-0 fs-4">Rp {{ number_format($hotel->startingPrice, 0, ',', '.') }}</h6></td>
                    <td class="border-bottom-0"><h6 class="fw-semibold mb-0 fs-4">Rp {{ number_format($hotel->price, 0, ',', '.') }}</h6></td>
                    <td class="border-bottom-0"><p class="mb-0 fw-normal">{{ $hotel->checkInTime }}</p></td>
                    <td class="border-bottom-0"><p class="mb-0 fw-normal">{{ $hotel->checkOutTime }}</p></td>
                    <td class="border-bottom-0">
                      <a href="{{ route('admin.hotels.informasi.edit', $hotel->id) }}" class="btn btn-warning btn-sm">
                          <i class="ti ti-pencil"></i> 
                      </a>
                      <form action="{{ route('admin.hotels.informasi.destroy', $hotel->id) }}" method="POST" style="display:inline;">
                          @csrf
                          @method('DELETE')
