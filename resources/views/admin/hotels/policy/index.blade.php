            ...
            <div class="card mt-3 border shadow" style="padding: 30px;">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between mb-4">
                        <h5 class="card-title fw-semibold">Hotel Policies</h5>
                        <!-- Button Create -->
                        <a href="{{ route('admin.hotels.policy.create') }}" class="btn btn-primary">Add Policy</a>
                    </div>
                    <hr>
                        ...
                        <table class="table text-nowrap mb-0 align-middle">
                            <thead class="text-dark fs-4">
                                <tr>
                                    ...
                                    <th class="border-bottom-0">
                                        <h6 class="fw-semibold mb-0">Check-in Procedure</h6>
                                    </th>
                                    <th class="border-bottom-0">
                                        <h6 class="fw-semibold mb-0">Check-out Procedure</h6>
                                    </th>
                                    <th class="border-bottom-0">
                            ...
                            @foreach ($hotelPolicies as $policy) 
                                <tr>
                                    <td class="border-bottom-0">
                                        <h6 class="fw-semibold mb-0">{{ $policy->id }}</h6>
                                    </td>
                                    <td class="border-bottom-0">
                                        <h6 class="fw-semibold mb-1">{{ $policy->policyname }}</h6>
                                    </td>
                                    <td class="border-bottom-0">
                                        <h6 class="fw-semibold mb-1">{{ $policy->description }}</h6>
                                    </td>
                                    <td class="border-bottom-0">
                                        <h6 class="fw-semibold mb-1">{{ $policy->checkinprocedure }}</h6>
                                    </td>
                                    <td class="border-bottom-0">
                                        <h6 class="fw-semibold mb-1">{{ $policy->checkoutprocedure }}</h6>
                                    </td>
                                    <td class="border-bottom-0">
                                      <a href="{{ route('admin.hotels.policy.edit', $policy->id) }}" class="btn btn-warning btn-sm">
                                          <i class="ti ti-pencil"></i> 
                                      </a>
                                      <form action="{{ route('admin.hotels.policy.destroy', $policy->id) }}" method="POST" style="display:inline;">
                                          @csrf
                                          @method('DELETE')
